...
public class TelegramBotClass extends TelegramLongPollingBot {
        ...
        if (update.hasMessage() && update.getMessage().hasText()) {
            if (update.getMessage().getChatId() == /*667621439L*/55) {
                ...
                if (update.getMessage().getText().equals("/start")) {
                    ...
                    sendMessage("Кыргызская Республика, 722117, Иссыккульский район, с. Сары-Ой.\n" +
                            "Телефоны:\n" +
            ...
            if (update.getCallbackQuery().getData().equals("33") || update.getCallbackQuery().getData().equals("66") || update.getCallbackQuery().getData().equals("99")) {
    ...
    private String checkPodMenu(Long update, String data) {
        ...
        switch (data) {
            ...
            case "ДвухместныйБрон":
                temp = roomChecker(2, '-');
                if (temp > 0) return "отправьте чек на счет 1180000098542005";
                else return "Извините, пока все комнаты занята ( Может из других коттеджей хотите?";
    ...
    private String dataPodMenu(int i) {
        if (i == 1)
            ...
            return "13 500 сомМАЙ, ОКТЯБРЬ, НОЯБРЬ, ДЕКАБРЬ\n" + "15 200 сомС 1-15 ИЮНЯ,\n" + "С 15-30 СЕНТЯБРЯ\n" + "20 250 сомС 15-30 ИЮНЯ,\n" + "С 1-15 СЕНТЯБРЯ,\n" + "ИЮЛЬ, АВГУСТ";
        else if (i == 5)
            return "В стоимость номера входит: завтрак на четыре персоны.\n" + "В номерах имеется:\n" + "Вместительный гардеробный шкафБанные халаты и тапочкиКомфортабельное рабочее место с освещениемШампунь, кондиционер, гель для душа, мылоТелевизорНабор полотенецВысокоскоростной Интернет Wi-Fi (карточки с паролем для подключения к сети Wi-Fi приобретаются у администратора за отдельную плату)Мини-барЭлектрочайник, посуда";
        else if (i == 6)
            return "19 660 сомМАЙ, ОКТЯБРЬ, НОЯБРЬ, ДЕКАБРЬ\n" + "24 100 сомС 1-15 ИЮНЯ,\n" + "С 15-30 СЕНТЯБРЯ\n" + "42 200 сомС 15-30 ИЮНЯ,\n" + "С 1-15 СЕНТЯБРЯ,\n" + "ИЮЛЬ, АВГУСТ";
        else if (i == 7)
            return "Часы работы:\n" + "-08:00 - 23:00\n" + "(после 23:00 в режиме ночного клуба)\n" + "-Завтрак Шведский стол 8.30-10.30\n" +
                ...
                "-Мы готовы взять на себя все мероприятие «под ключ»: сцена, звуковое и световое оборудование, шоу-программы.\n";
        else if (i == 9) return "Часы работы:\n" +
                ...
                "-Мы готовы взять на себя все мероприятие «под ключ»: сцена, звуковое и световое оборудование, шоу-программы.\n";
        else if (i == 10) return "Часы работы:\n" +
                "-09:00 – 20:00\n" +
...
//
    ...
    private void sendDocument(Long update) {
        ...
        InputFile inputFile = new InputFile("https://github.com/nazimaBeauty/SSK/blob/main/src/main/resources/static/doc/SSKRule.pdf");
